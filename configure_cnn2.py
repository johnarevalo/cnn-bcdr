...
for job_id in xrange(num_jobs):
    ...
    if os.path.isfile(paths['cnn_layer_2']['model_yaml']):
        print '%s already exists, skipping...' % (paths['cnn_layer_2']['model_yaml'])
        continue

    # Initialization params
    conf['h0']['nhid'] = rng.randint(32, 64)
    conf['h0']['init'] = random_init_string()
    conf['h0']['bias'] = rectifier_bias()
    conf['hfc']['nhid'] = rng.randint(64, 512)
    conf['hfc']['init'] = random_init_string(sparse=True)
    if rng.randint(2):
        conf['y']['init'] = "sparse_init: 0"
    else:
    ...
    # SGD params
    conf['learning_rate'] = 10. ** rng.uniform(-2., -.5)
    ...
    with open(paths['cnn_layer_2']['model_yaml'], 'w') as f:
        yaml.dump(conf, f)
